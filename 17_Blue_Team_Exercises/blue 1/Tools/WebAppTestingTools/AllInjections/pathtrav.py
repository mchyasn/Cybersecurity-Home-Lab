...
def modify_request_for_path_traversal(request, payload):
    lines = request.split('\n')
    modified = []
    for line in lines:
        if line.startswith('GET') or line.startswith('POST') or line.startswith('PUT') or line.startswith('DELETE'):
            ...
            for part in url_parts:
                if part:  # Skip empty parts
                    modified_url_parts.append(part + payload)
            modified_url = '/'.join(modified_url_parts)
            if not modified_url.startswith('/'):
...
def scan_for_path_traversal(xml_file, business, asset_type, asset_value):
    base_output_dir = "/home/kali/Desktop/BugBountyWork"
    wordlist_file = "/usr/share/wfuzz/wordlist/vulns/dirTraversal-nix.txt"
    injection_type = "PathTraversal"
    output_dir = os.path.join(base_output_dir, business, asset_type, asset_value,
                              "WEBENVFINAL", "Server-Side Topics", "file-path-traversal", "Techniques", "basic-path-traversal")
            ...
            if request_element is not None and request_element.text:
                ...
                if any(method in first_line for method in ['GET', 'POST', 'PUT', 'DELETE']):
                    ...
                    for j in range(3):
                        ...
                        modified_request = modify_request_for_path_traversal(request, payload)
...
if __name__ == '__main__':
    ...
    try:
        scan_for_path_traversal(args.xml_file, args.business, args.asset_type, args.asset_value)
    except Exception as e:
