...
def modify_request_for_sql_injection(request, payload):
    lines = request.split('\n')
    modified = []
    for line in lines:
...
def scan_for_sql_injection(xml_file, business, asset_type, asset_value):
    base_output_dir = "/home/kali/Desktop/BugBountyWork"
    wordlist_file = "/usr/share/wfuzz/wordlist/Injections/SQL.txt"
    injection_type = "SQLInjection"
    output_dir = os.path.join(base_output_dir, business, asset_type, asset_value,
                              "WEBENVFINAL", "Server-Side-Topics", "sql-injection", "Techniques", "basic-sql-injection")
            ...
            if request_element is not None and request_element.text:
                ...
                if any(method in first_line for method in ['GET', 'POST', 'PUT', 'DELETE']):
                    ...
                    for j in range(3):
                        ...
                        try:
                            modified_request = modify_request_for_sql_injection(request, payload)
                            modified_file = os.path.join(output_dir, f'modified_request_{i + 1}_{j + 1}.txt')
                            with open(modified_file, 'w') as f:
                                f.write(modified_request)
                        except Exception as e:
                            logging.error(f"Error modifying request {i + 1} with payload {j + 1}: {str(e)}")
                            continue
...
if __name__ == '__main__':
    ...
    try:
        scan_for_sql_injection(args.xml_file, args.business, args.asset_type, args.asset_value)
